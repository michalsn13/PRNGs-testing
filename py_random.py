...
for par in params:
    ...
    print(f'Testing for py_random{par} pseudo-random sequence with x0={x0}...')
    random.seed(x0)
    sequence=np.array([random.randint(0,par[0]-1) for _ in range(n*R)])
    ...
    plt.suptitle(f'py_random{par} pseudo-random sequence with x0={x0}\n{n} first iterations')
    plt.savefig(f'py_random/py_random{plot_num}.png')
    fig, axs = plt.subplots(1, 2, figsize=(16, 5))
