...
for par in params:
    ...
    L=par[1]
    print(f'Testing for one key RC4{par} pseudo-random sequence...')
    key = []
    value=10000
    for _ in range(L):
        sleep(value/100000)
    ...
    key = np.array(key)
    sequence=prngs.RC4(n*R,par[0],key)
    ...
    fig.suptitle(f'One key RC4{par} sequence\n{n} first iterations')
    plt.savefig(f'RC4_one_key/RC4_one_key{plot_num}.png')
    fig, axs = plt.subplots(1, 2, figsize=(20, 5))
    for test in all_tests:
        ...
        try:
            ...
            axs[plot_num % 2].set_title(
                f'One key RC4{par} p-values from {test["name"]} test with 2-nd level testing p-value={round(p_value_2nd, 4)}')
            plot_num += 1
            if plot_num % 2 == 0:
                plt.savefig(f'RC4_one_key/RC4_one_key{plot_num}.png')
                fig, axs = plt.subplots(1, 2, figsize=(20, 5))